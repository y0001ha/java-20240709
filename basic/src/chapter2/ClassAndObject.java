...
class ExampleClass1 {
	...
	static void staticMthod() {
		//System.out.println(attribute1); -->> 불가
		//method1(5); 
	}
}
...
class SmartPhone {
	...
	String manufacturer;
	String manufacuterDate;
	String modelName;
	String telNumber;
	String[] appList = new String[0];
	boolean powerStatus;
	
	void printInformation() { //-->> 반환값이 없으니 void 
		System.out.println("===== 정보 =====");
		System.out.println("제조사 : " + manufacturer);
		System.out.println("제조일자 : " + manufacuterDate);
		System.out.println("모델명 : " + modelName);
		System.out.println("전화번호 : " + telNumber);
	}
	
	void call(String toTelNumber) { //-->> (요구하는것)
...
class TriangleMath {
	...
	static double getHypotenuse(double bottom, double height) {
		...
		double hypotenuse2 = Math.pow(bottom, 2) + Math.pow(height, 2);// 2제곱
		double hypotenuse = Math.sqrt(hypotenuse2);// 제곱근
...
public class ClassAndObject {
	...
	public static void main(String[] args) {
		...
		// 인스턴스가 가지고 있는 속성 접근 방법 (필드에 접근한다고 표현)
		// 인스턴스.속성명;
		instance1.attribute1 = 10;
		instance2.attribute1 = 20;
		// new ExampleClass1().attribute1 = 30;  -->> instance1은 변수, 인스턴스는 new ExampleClass1()
		...
		iphone1.manufacuterDate = "2024-07-12";
		...
		korean1.name = "홍길동";
		korean1.age = 20;
		//korean1.nationality = "대한민국"; --> 위에 초기값을 넣어줘서 안써도 출력이 됨
		...
		// static 붙였기때문에 아래처럼 생성작업이 필요 없다@@@@
