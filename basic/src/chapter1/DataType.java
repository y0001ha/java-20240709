...
public class DataType {
	...
	public static void main(String[] args) {
		...
		int int1 = 32768;
		int1 = 2_147_483_647;
		// 정수형 리터럴 상수는 int 타입임 --> 뒤에 L 넣어줘야함 @@@-------
		//int1 = 2_147_483_648L; --> 범위 초과
		...
		// 정수형 int를 연산 결과를 저장하는 데이터 타입으로 사용할 때는 연산 결과가 범위를 초과할 수 있는지 주의할 필요가 있음 
		...
		// 소수점 6-7자리에서 오차가 발생
		// 실수형 리터럴 상수는 double 타입임 --> 뒤에 F 넣어줘야함 @@@-------
	    ...
	    int number = 300;
	    long long2 = number;
	    //number = long2; --->> 사이즈가 달라서 안됨
	    number = (int) long2; // --->> 강제 형변환
	    
	    char char3 = 100;
	    //short short2 = char3; // --->> 부호부분 때문에 안됨
	    short short2 = (short) char3; // --->> 강제 형변환
	    int int2 = char3;
	    ...
	    int[] numbers;
	    // numbers = 3; -->> 단순 정수는 안됨
	    numbers = new int[5]; // -> 주소를 가지고 있음
	    
	    int[] numbers2 = {1,2,3,4,5}; // -> 주소를 가지고 있음
	    int[] numbers3 = new int[] {1,2,3,4,5}; // -> 주소를 가지고 있음
	    ...
	    // 인덱스는 0번부터 시작, 마지막 인덱스는 길이의 -1번 (책갈피)
	    int item = numbers3[2];
	    System.out.println(item);
	    
	    numbers3[0] = 9;  //--->>> numbers3이라는 배열에 0이라는 인덱스에 접근한다.라고 표현
	    item = numbers3[0]; // 1을 9로 바꾸고 싶을 때
	    System.out.println(item);
	    ...
	    int[] example2 = example1; // new를 안써서 같은 공간 참조
	    ...
	    // 변수명 = "문자열";
	    String string1;
	    string1 = new String(); //--> 의마가 없음. 의마가 있으려면 string1 = new String("apple");
	    string1 = "apple";
	    ...
	    // compareToIgnoreCase : 두 문자열을 사전순으로 비교 (대소문자 구분X)
