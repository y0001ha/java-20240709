...
public class Package2 {
	...
	public static void main(String[] args) {
		...
		list = new ArrayList<>();    //list = new ArrayList<Integer>(); 도 가능	
		...
		// 기본형 데이터 타입 변수로 반환값을 받을 땐 null 이 올수 있음을 주의
		int number = list.get(0);
		System.out.println(number); //-->> int는 null 못받음 integer는 참조형이라 가능
		...
		// get() : Map에서 특정 키를 가지는 요소르 반환  -2번
		// 존재하지 않는 키를 검색하면 null을 반환
		System.out.println(map.get("Apple"));
		
		// containsKey() : Map에 특정 키가 존재하는지 확인  -1번
		...
		human2.put("naem", "홍길동"); //1/문제점
		human2.put("address", "부산광역시");
		human2.put("age", 20);
		
		// 1. 문제점 : 사용 혹은 추가 작업에서 실수 가능성
		human2.get("name");
		// 2. 문제점 : 타입의 안정성의 문제
