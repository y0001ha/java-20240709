...
// 아래와 같은 Test1, Test2, Test3 형태로 클래스를 작성하면 코드 중복 발생
class Test1 {
	String subject;
	int score;
}
...
public class Generic {
	...
	// 제너릭 메서드 : 해당 메서드의 매개변수 타입이나 반환 타입을 컴파일시에 결정하고자 할 때 사용
	// <매개타입> 반환타입 메서드명 (매개변수) { ... }
	...
	// ? extends 클래스명 : 특정 클래스 혹은 클정 클래스를 상속받은 클래스만 타입으로 받을 수 있도록 제한
	...
	public static void main(String[] args) {
		...
		Test5<Integer> instance2 = new Test5<Integer>(); //-->> 이 시점에 결정 변수의 매개타입
		instance2.score = 90;
		...
		//ExtendsClass<SubClass> instance3 = new ExtendsClass<>(); -->> 불가
		...
		// MyClass 혹은 MyClass의 부모 클래스만 매개변수로 전달이 가능
		superMethod(list4);
		superMethod(list3);
		//superMethod(list5);
		//superMethod(list2);
